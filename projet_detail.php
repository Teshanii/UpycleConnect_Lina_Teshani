...
function afficherEntete(p) {
    ...
    html += '<h3 style="color:var(--primary-green);">' + p.titre + '</h3>';
    html += '<p class="text-muted mb-1">Créé par <strong>' + p.createur + '</strong></p>';
    if (p.ville || p.adresse) {
        html += '<p class="text-muted mb-2"> ' + (p.adresse ? p.adresse + ', ' : '') + (p.ville || '') + '</p>';
    }
...
fetch('http://localhost:8080/api/etapes?id_projet=' + idProjet)
    ...
    .then(function(data) {
        ...
        data.forEach(function(e, index) {
            ...
            html += '    <h6 style="color:var(--primary-green);">Étape ' + (index + 1) + ' : ' + (e.titre || '') + '</h6>';
            if (img) {
                html += '    <img src="' + img + '" class="w-100 mb-2" style="max-height:350px; object-fit:cover; border-radius:8px;">';
            }
            html += '    <p class="mb-0">' + (e.description || '') + '</p>';
            html += '  </div>';
            html += '</div>';
        });
        zone.innerHTML = html;
...
fetch('http://localhost:8080/api/participants?id_projet=' + idProjet)
    ...
    .then(function(data) {
        ...
        acceptes.forEach(function(p) {
            ...
            html += '<strong>' + p.nom_user + '</strong>';
            if (p.tache) {
                html += ' — <span class="text-muted">' + p.tache + '</span>';
            }
            html += '</li>';
        });
        html += '</ul>';
